...
func newFile(db *gorm.DB, opts ...gen.DOOption) file {
	...
	_file.fileDo.UseModel(&gorm_gen.File{})
...
type file struct {
	...
	Pids      field.String // 上级ID递归
	Cid       field.Int64  // 复制ID
	Name      field.String // 名称
	Type      field.String // 类型
	Ext       field.String // 后缀名
	Size      field.Int64  // 大小(B)
	Userid    field.Int64  // 拥有者ID
	Share     field.Int32  // 是否共享
	Pshare    field.Int64  // 所属分享ID
...
type IFileDo interface {
	...
	Unscoped() IFileDo
	Create(values ...*gorm_gen.File) error
	CreateInBatches(values []*gorm_gen.File, batchSize int) error
	Save(values ...*gorm_gen.File) error
	First() (*gorm_gen.File, error)
	Take() (*gorm_gen.File, error)
	Last() (*gorm_gen.File, error)
	Find() ([]*gorm_gen.File, error)
	FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*gorm_gen.File, err error)
	FindInBatches(result *[]*gorm_gen.File, batchSize int, fc func(tx gen.Dao, batch int) error) error
	Pluck(column field.Expr, dest interface{}) error
	Delete(...*gorm_gen.File) (info gen.ResultInfo, err error)
	...
	FirstOrInit() (*gorm_gen.File, error)
	FirstOrCreate() (*gorm_gen.File, error)
	FindByPage(offset int, limit int) (result []*gorm_gen.File, count int64, err error)
...
func (f fileDo) Create(values ...*gorm_gen.File) error {
	if len(values) == 0 {
		return nil
	}
...
func (f fileDo) CreateInBatches(values []*gorm_gen.File, batchSize int) error {
	return f.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (f fileDo) Save(values ...*gorm_gen.File) error {
	if len(values) == 0 {
		return nil
	}
...
func (f fileDo) First() (*gorm_gen.File, error) {
	if result, err := f.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*gorm_gen.File), nil
	}
}

func (f fileDo) Take() (*gorm_gen.File, error) {
...
func (f fileDo) Last() (*gorm_gen.File, error) {
...
func (f fileDo) Find() ([]*gorm_gen.File, error) {
	result, err := f.DO.Find()
	return result.([]*gorm_gen.File), err
}

func (f fileDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*gorm_gen.File, err error) {
	buf := make([]*gorm_gen.File, 0, batchSize)
...
func (f fileDo) FindInBatches(result *[]*gorm_gen.File, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (f fileDo) FirstOrInit() (*gorm_gen.File, error) {
...
func (f fileDo) FirstOrCreate() (*gorm_gen.File, error) {
...
func (f fileDo) FindByPage(offset int, limit int) (result []*gorm_gen.File, count int64, err error) {
...
func (f fileDo) Delete(models ...*gorm_gen.File) (result gen.ResultInfo, err error) {
